...
int main()
{
    ...
    std::for_each(intersections.begin(), intersections.end(), [](shared_ptr<Intersection> &i) {
        i->simulate();
    });

    // simulate vehicles
    std::for_each(vehicles.begin(), vehicles.end(), [](shared_ptr<Vehicle> &v) {
        v->simulate();
    });
    ...
    std::for_each(intersections.begin(), intersections.end(), [&trafficObjects](shared_ptr<Intersection> &intersection) {
        shared_ptr<TrafficObject> trafficObject = std::dynamic_pointer_cast<TrafficObject>(intersection);
        trafficObjects.push_back(trafficObject);
    });

    std::for_each(vehicles.begin(), vehicles.end(), [&trafficObjects](shared_ptr<Vehicle> &vehicles) {
        shared_ptr<TrafficObject> trafficObject = std::dynamic_pointer_cast<TrafficObject>(vehicles);
        trafficObjects.push_back(trafficObject);
    });
