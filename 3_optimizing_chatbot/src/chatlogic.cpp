...
void ChatLogic::AddAllTokensToElement(string tokenID, tokenlist &tokens, T &element)
{
    ...
    while (true)
    {
        token = std::find_if(token, tokens.end(), [&tokenID](const std::pair<string, string> &pair) { return pair.first == tokenID;; });
        if (token != tokens.end())
        {
...
void ChatLogic::LoadAnswerGraphFromFile(string filename)
{
    ...
    if (file)
    {
        ...
        while (getline(file, lineStr))
        {
            ...
            auto type = std::find_if(tokens.begin(), tokens.end(), [](const std::pair<string, string> &pair) { return pair.first == "TYPE"; });
            if (type != tokens.end())
            {
                // check for id
                auto idToken = std::find_if(tokens.begin(), tokens.end(), [](const std::pair<string, string> &pair) { return pair.first == "ID"; });
...
{
    this->_panelDialog->PrintChatbotResponse(message);
}
