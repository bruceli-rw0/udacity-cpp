...
bool path_comp(RouteModel::Node *nodeA, RouteModel::Node *nodeB) {
    // lowest path node will be at the end
    return (nodeA->g_value + nodeA->h_value) > (nodeB->g_value + nodeB->h_value);
}

RouteModel::Node *RoutePlanner::NextNode() {
    std::sort(this->open_list.begin(), this->open_list.end(), path_comp);
    // auto lowest_node = this->open_list[this->open_list.size() - 1];
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    assert (current_node == this->end_node);
    ...
    // reverse path_found to from start to end
